                      ...
                      k_u8 value, k_u8 pad_value)
{
    ...
    for (y = 0; y < height; ++y) {
        memset(buf + y * stride, value, width);
        if (stride > width)
            memset(buf + y * stride + width, pad_value, stride - width);
    }
}
...
int main(void)
{
    ...
    EXPECT_TRUE(ret == 0, "first frame should be accepted");
    EXPECT_TRUE(result.is_motion == 0, "first frame should seed baseline only");
    ...
    ret = motion_detect_process(&frame, &result);
    EXPECT_TRUE(ret == 0, "same luma with changed padding should be accepted");
    ...
    fill_luma(changed_luma, STRIDE, WIDTH, HEIGHT, 95, 0xee);
    frame.frame_id = 3;
    frame.y = changed_luma;
    ret = motion_detect_process(&frame, &result);
    EXPECT_TRUE(ret == 0, "changed luma frame should be accepted");
    EXPECT_TRUE(result.is_motion != 0, "large luma delta should trigger motion");
    EXPECT_TRUE(result.motion_score > 0, "motion score should be positive");
    ...
    if (g_failures) {
        printf("%d motion default test(s) failed\n", g_failures);
        return 1;
    }

    printf("motion default tests passed\n");
    return 0;
}
