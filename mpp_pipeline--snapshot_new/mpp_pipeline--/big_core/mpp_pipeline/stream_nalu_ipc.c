...
static k_u32 g_invalid_stream_drop_count;
static k_u32 g_compacted_stream_count;
...
static k_bool nalu_ipc_pack_is_valid(const k_venc_pack *pack)
{
    return pack && pack->phys_addr != 0 && pack->len != 0;
}

static k_u32 nalu_ipc_count_valid_packs(const k_venc_stream *stream)
{
    k_u32 valid_cnt = 0;
    ...
    for (k_u32 i = 0; i < stream->pack_cnt && i < MPP_NALU_IPC_MAX_PACKS; i++) {
        if (nalu_ipc_pack_is_valid(&stream->pack[i]))
            valid_cnt++;
    }
                               ...
                               k_u32 flags)
{
    ...
    for (k_u32 i = 0; i < stream->pack_cnt && i < MPP_NALU_IPC_MAX_PACKS; i++) {
        ...
        if (!nalu_ipc_pack_is_valid(pack))
            continue;

        if (out_index == 0)
            msg->frame_pts = pack->pts;

        msg->packs[out_index].phys_addr = pack->phys_addr;
        msg->packs[out_index].pts = pack->pts;
        msg->packs[out_index].len = pack->len;
        msg->packs[out_index].type = pack->type;
        msg->total_len += pack->len;
        out_index++;
    }

    msg->pack_cnt = out_index;
}
                             ...
                             k_u32 flags)
{
    ...
    if (nalu_ipc_count_valid_packs(stream) == 0) {
        g_invalid_stream_drop_count++;
        if (g_invalid_stream_drop_count == 1 ||
            (g_invalid_stream_drop_count % 30) == 0) {
            LOG("NALU IPC drop empty stream: pack_cnt=%u first_phys=0x%llx first_len=%u drops=%u",
                stream->pack_cnt,
                (stream->pack_cnt > 0) ?
                    (unsigned long long)stream->pack[0].phys_addr : 0ULL,
                (stream->pack_cnt > 0) ? stream->pack[0].len : 0,
                g_invalid_stream_drop_count);
        }
        return -1;
    }
    ...
    nalu_ipc_build_msg(&pending->ipc_msg, chn, stream, flags);
    if (pending->ipc_msg.pack_cnt == 0 || pending->ipc_msg.total_len == 0) {
        LOG("NALU IPC build empty msg from stream pack_cnt=%u", stream->pack_cnt);
        memset(pending, 0, sizeof(*pending));
        return -1;
    }
    if (pending->ipc_msg.pack_cnt != stream->pack_cnt) {
        g_compacted_stream_count++;
        if (g_compacted_stream_count == 1 ||
            (g_compacted_stream_count % 30) == 0) {
            LOG("NALU IPC compact stream packs: raw=%u valid=%u total_len=%u count=%u",
                stream->pack_cnt,
                pending->ipc_msg.pack_cnt,
                pending->ipc_msg.total_len,
                g_compacted_stream_count);
        }
    }
